package com.example.construction_project.exception;

public class ErrorMessage {

    public static final String OWNERS_EMPTY = "OWNERS_EMPTY";
    public static final String USERS_EMPTY = "USERS_EMPTY";
    public static final String BUILDING_NOT_EXIST = "BUILDING_NOT_EXIST";
    public static final String USER_NOT_EXIST = "USER_NOT_EXIST";
    public static final String BUILDINGS_EMPTY = "BUILDINGS_EMPTY";
    public static final String BUILDING_ALREADY_EXIST = "BUILDING_ALREADY_EXIST";
    public static final String USER_ALREADY_EXIST = "USER_ALREADY_EXIST";
    public static final String OWNER_ALREADY_EXIST = "OWNER_ALREADY_EXIST";
    public static final String OWNER_NOT_EXIST = "OWNER_NOT_EXIST";
}
